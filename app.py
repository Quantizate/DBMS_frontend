...
def get_column_names(table_name):
    ...
    columns = [
        column[0] for column in mycursor.fetchall()
...
def check_equipment():
    ...
    if not existing_id:
        return (
            jsonify({"Availability": "False", "Message": "No such equipment exists."}),
            400,
        )
    elif existing_id[1] == 0:
        return (
            jsonify(
                {
                    "Availability": "False",
                    "Message": "The selected equipment is unavailable at the moment.",
                }
            ),
            400,
        )
...
def fetch_student_courses(email):
    ...
    roll_no = roll_no[0]
    cur.execute(
        "SELECT Course_ID FROM student_enrolled WHERE Roll_Number = %s", (roll_no,)
    )
    courses = cur.fetchall()  # Fetch all rows
    return courses
